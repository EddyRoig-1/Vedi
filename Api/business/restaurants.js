...
/**
 * Generate and save QR codes for a restaurant
 * @param {string} restaurantId - The restaurant ID
 * @param {string} baseMenuUrl - Base URL for the customer menu
 * @returns {Promise<Object>} The generated QR data
 */
async function generateRestaurantQRCodes(restaurantId, baseMenuUrl) {
    const startTime = performance.now();
    
    try {
        console.log(`🔄 Generating QR codes for restaurant: ${restaurantId}`);
        
        // Get restaurant data to determine table count
        const restaurant = await getRestaurant(restaurantId);
        if (!restaurant) {
            throw new Error('Restaurant not found');
        }
        
        const tableCount = parseInt(restaurant.tableCount) || 0;
        
        // Check if QR codes already exist
        const existingQRData = await loadRestaurantQRCodes(restaurantId);
        
        if (existingQRData && existingQRData.tableCount === tableCount) {
            console.log(`✅ QR codes already exist and table count matches: ${tableCount}`);
            return existingQRData;
        }
        
        // Generate new QR codes or add missing ones
        let qrData;
        if (!existingQRData) {
            // First time generation
            qrData = await createInitialQRCodes(restaurantId, tableCount, baseMenuUrl);
        } else {
            // Update existing QR codes
            qrData = await updateQRCodes(restaurantId, existingQRData, tableCount, baseMenuUrl);
        }
        
        const endTime = performance.now();
        console.log(`✅ QR codes generated successfully in ${(endTime - startTime).toFixed(2)}ms`);
...
/**
 * Load existing QR codes for a restaurant
 * @param {string} restaurantId - The restaurant ID
 * @returns {Promise<Object|null>} Existing QR data or null
 */
async function loadRestaurantQRCodes(restaurantId) {
    try {
        const db = firebase.firestore();
        const qrRef = db.collection('restaurants')
            .doc(restaurantId)
            .collection('qrCodes')
            .doc('persistent');
        ...
        if (doc.exists) {
            const data = doc.data();
            console.log(`✅ Loaded existing QR codes for restaurant: ${restaurantId}`);
            return data;
        }
        
        console.log(`ℹ️ No existing QR codes found for restaurant: ${restaurantId}`);
        return null;
...
/**
 * Create initial QR codes for a restaurant
 * @param {string} restaurantId - The restaurant ID
 * @param {number} tableCount - Number of tables
 * @param {string} baseMenuUrl - Base URL for the customer menu
 * @returns {Promise<Object>} The generated QR data
 */
async function createInitialQRCodes(restaurantId, tableCount, baseMenuUrl) {
    try {
        console.log(`🆕 Creating initial QR codes for ${tableCount} tables`);
        ...
        // Generate counter QR
        qrData.urls.counter = `${baseMenuUrl}?restaurant=${restaurantId}&table=counter&location=counter`;
        
        // Generate table QRs
        for (let i = 1; i <= tableCount; i++) {
            qrData.urls[`table_${i}`] = `${baseMenuUrl}?restaurant=${restaurantId}&table=${i}&location=table`;
        }
        
        // Save to Firebase using batch write for reliability
        const db = firebase.firestore();
        ...
        console.log(`✅ Initial QR codes saved for restaurant: ${restaurantId}`);
        
        // Convert timestamp for return
        return {
            ...qrData,
            generatedAt: qrData.generatedAt.toDate().toISOString()
        };
        
    } catch (error) {
        console.error('❌ Error creating initial QR codes:', error);
        throw error;
    }
}

/**
 * Update existing QR codes (add/remove tables)
 * @param {string} restaurantId - The restaurant ID
 * @param {Object} existingQRData - Current QR data
 * @param {number} newTableCount - New table count
 * @param {string} baseMenuUrl - Base URL for the customer menu
 * @returns {Promise<Object>} Updated QR data
 */
async function updateQRCodes(restaurantId, existingQRData, newTableCount, baseMenuUrl) {
    try {
        const oldTableCount = existingQRData.tableCount || 0;
        
        if (newTableCount > oldTableCount) {
            // Add new tables
            console.log(`📈 Adding tables ${oldTableCount + 1} to ${newTableCount}`);
            
            for (let i = oldTableCount + 1; i <= newTableCount; i++) {
                existingQRData.urls[`table_${i}`] = `${baseMenuUrl}?restaurant=${restaurantId}&table=${i}&location=table`;
            }
        }
        
        // Update metadata
        existingQRData.tableCount = newTableCount;
        existingQRData.lastUpdated = firebase.firestore.Timestamp.now();
        ...
        await qrRef.set(existingQRData, { merge: true });
        
        console.log(`✅ QR codes updated for restaurant: ${restaurantId}`);
        
        // Convert timestamps for return
        return {
            ...existingQRData,
            generatedAt: existingQRData.generatedAt.toDate ? existingQRData.generatedAt.toDate().toISOString() : existingQRData.generatedAt,
            lastUpdated: existingQRData.lastUpdated.toDate().toISOString()
        };
...
/**
 * Delete QR codes for a restaurant
 * @param {string} restaurantId - The restaurant ID
 * @returns {Promise<boolean>} Success status
 */
async function deleteRestaurantQRCodes(restaurantId) {
    try {
        console.log(`🗑️ Deleting QR codes for restaurant: ${restaurantId}`);
        ...
        console.log(`✅ QR codes deleted for restaurant: ${restaurantId}`);
        return true;
...
Object.assign(window.VediAPI, {
  ...
  loadRestaurantQRCodes,
  createInitialQRCodes,
  updateQRCodes,
...
console.log('📱 QR Codes: generateRestaurantQRCodes, loadRestaurantQRCodes, createInitialQRCodes, updateQRCodes, deleteRestaurantQRCodes');
